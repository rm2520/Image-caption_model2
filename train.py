...
def train_epoch(model, train_loader, tokenizer, criterion, optim, epoch, device):
    ...
    for i, batch in bar:
        ...
        caps = [tokenizer.decode(cap, skip_special_tokens=False) for cap in preds]
...
def validate_epoch(model, valid_loader, tokenizer, criterion, epoch, device):
    model.eval()
    total_loss, batch_bleu4 = [], []
    batch_bleu1,batch_bleu2,batch_bleu3=[],[],[]
    hypotheses, references = [], []
    with torch.no_grad():
        ...
        for i, batch in bar:
            ...
            for i in range(batch_size):
                ri = [all_caps[j][i].split() for j in range(len(all_caps)) if all_caps[j][i]]
                ref.append(ri)
            batch_bleu4.append(corpus_bleu(ref, hypo,weights=(0.25, 0.025, 0.25, 0.25)))
            batch_bleu3.append(corpus_bleu(ref, hypo, weights=(0.333, 0.333, 0.333, 0)))
            batch_bleu2.append(corpus_bleu(ref, hypo, weights=(0.5, 0.5, 0, 0)))
            batch_bleu1.append(corpus_bleu(ref, hypo, weights=(1, 0, 0, 0)))
            hypotheses += hypo
            references += ref

            bar.set_postfix(loss=total_loss[-1], bleu1=batch_bleu1[-1])

    val_loss = sum(total_loss) / len(total_loss)
    val_bleu4 = corpus_bleu(references, hypotheses, smoothing_function=smoothie.method4)
    val_bleu3 = corpus_bleu(references, hypotheses, weights=(0.333, 0.333, 0.333, 0),smoothing_function=smoothie.method4)
    val_bleu2 = corpus_bleu(references, hypotheses,weights=(0.5, 0.5, 0, 0), smoothing_function=smoothie.method4)
    val_bleu1 = corpus_bleu(references, hypotheses, weights=(1, 0, 0, 0),smoothing_function=smoothie.method4)
    print("blue1 : ",val_bleu1," blue2 :",val_bleu2," blue3 :",val_bleu3," blue4: ",val_bleu4)
...
def main():
    ...
    parser.add_argument("--tokenizer", "-t", type=str, default="asafaya/bert-base-arabic", help="Bert tokenizer")
    ...
    parser.add_argument("--n_epochs", "-ne", type=int, default=33, help="Number of epochs")
    ...
    # Data parameters
    parser.add_argument("--image_dir", "-id", type=str, default="data/", help="Path to image directory, this contains train2014, val2014")
    parser.add_argument("--karpathy_json_path", "-kap", type=str, default="data/Flickr8k_text/Flickr8k.arabic.full.txt", help="Path to karpathy json file")
    parser.add_argument("--val_annotation_path", "-vap", type=str, default="data/Flickr8k_text/Flickr_8k.devImages.txt", help="Path to validation annotation file")
    parser.add_argument("--train_annotation_path", "-tap", type=str, default="data/Flickr8k_text/Flickr_8k.trainImages.txt", help="Path to training annotation file")
    # Log parameters
