...
def evaluate():
    ...
    parser.add_argument("--karpathy_json_path", "-kap", type=str, default="data/jsonfiles/dataset_flickr8k.json", help="Path to karpathy json file")
    parser.add_argument("--val_annotation_path", "-vap", type=str, default="data/jsonfiles/dataset_flickr8k.json", help="Path to validation annotation file")
    ...
    image_paths = [os.path.join(args.image_dir, "Flicker8k_Dataset", image["filename"]) for image in kaparthy["images"] if image["split"] == "test"]
    image_ids = [image["imgid"] for image in kaparthy["images"] if image["split"] == "test"]
    # Evaluate model
    model.eval()
